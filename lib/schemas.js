...
function construct(self, schema) {
	...
	_.each(schema, function(spec, key) {
		if (spec.get || spec.set) return Object.defineProperty(self, key, { get: spec.get, set: spec.set, enumerable: true }); // spec is getter/setter
...
function canCast(val, spec)
{
  ...
  if (spec == "regexp" && util.isRegExp(val) && val.toString) return true;
	return false;
};
...
function castToType(val, spec)
{
  ...
  if (util.isRegExp(val)) return val;
};
